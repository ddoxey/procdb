...
std::optional<std::vector<Field>> Collect::get(const PayloadType type) {
  ...
  switch (type) {
    case PayloadType::PROCESS_STATS: {
      ...
      auto of_count = ShellCmd::run(
          "lsof -p $(pidof 'Google Chrome' | "
          "sed -e 's/[ ]*$//' -e 's/ /,/g') | wc -l");
    ...
    case PayloadType::NETWORK_STATS: {
      auto net_stats =
          ShellCmd::run("nettop -m tcp -l 1 | grep 'Google Chrome'");
      if (net_stats) {
    ...
    case PayloadType::PROCESS_LIST: {
      auto processes =
          ShellCmd::run("ps -ec -o pcpu,pmem,comm | grep 'Google Chrome'");
      if (processes) {
