...
void Field::Serialize(std::vector<char>& buffer) const
{
    ...
    buffer.insert(buffer.end(),
        reinterpret_cast<const char*>(&label_size),
    ...
    buffer.insert(buffer.end(),
        reinterpret_cast<const char*>(&has_value),
...
void Payload::Serialize(std::vector<char>& buffer) const
{
    ...
    buffer.insert(buffer.end(),
        reinterpret_cast<const char*>(&field_count),
...
Payload
Payload::Deserialize(const std::vector<char>& buffer)
{
    size_t offset = 0;
