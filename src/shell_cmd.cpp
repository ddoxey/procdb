...
{
    auto newEnd = std::unique(str.begin(), str.end(), [extra](char left, char right) {
...
ShellCmd::split(const std::string& str, char delimiter)
{
...
ShellCmd::Result
ShellCmd::run(const std::string& cmd)
{
    char buffer[4096];
    ...
    return { make_optional(stdout_data),
        make_optional(stderr_data),
        exit_status };
}
